...
void loop()
{
    ...
    int i = 0;
    float v3[5], v136[5], v137[5], Rs3[5], Rs136[5], Rs137[5], mq3_value[5], mq136_value[5], mq137_value[5] , slope3[4], slope136[4], slope137[4];
    while (i < 5)
    {
        v3[i] = readSensorVoltage(MQ3_PIN);
        v136[i] = readSensorVoltage(MQ136_PIN);
        v137[i] = readSensorVoltage(MQ137_PIN);

        Rs3[i] = calculateRs(v3[i]);
        Rs136[i] = calculateRs(v136[i]);
        Rs137[i] = calculateRs(v137[i]);

        mq3_value[i] = Rs3[i] / Ro3;
        mq136_value[i] = Rs136[i] / Ro136;
        mq137_value[i] = Rs137[i] / Ro137;
        i++;
        delay(300);//need to figure this out
    }
    int avg_mq3 = 0, avg_mq136 = 0, avg_mq137 = 0;
    for (int j = 0; j < 5; j++)
    {
        avg_mq3 += mq3_value[j];
        avg_mq136 += mq136_value[j];
        avg_mq137 += mq137_value[j];
    }
    avg_mq3 /= 5;
    avg_mq136 /= 5;
    avg_mq137 /= 5;
    Serial.printf("MQ-3 Ratio: %.3f | MQ-136 Ratio: %.3f | MQ-137 Ratio: %.3f\n",
                  avg_mq3, avg_mq136, avg_mq137);
    for(int i=0;i<4;i++){
        slope3[i] = (mq3_value[i+1] - mq3_value[i])*100/mq3_value[i];
        slope136[i] = (mq136_value[i+1] - mq136_value[i])*100/mq136_value[i];
        slope137[i] = (mq137_value[i+1] - mq137_value[i])*100/mq137_value[i];
    }
    int avgslope_mq3 = 0, avgslope_mq136 = 0, avgslope_mq137 = 0;
    for (int j = 0; j < 4; j++)
    {
        avgslope_mq3 += slope3[j];
        avgslope_mq136 += slope136[j];
        avgslope_mq137 += slope137[j];
    }
    avgslope_mq3 /= 4;
    avgslope_mq136 /= 4;
    avgslope_mq137 /= 4;
    Serial.printf("MQ-3 Slope: %.3f%% | MQ-136 Slope: %.3f%% | MQ-137 Slope: %.3f%%\n",
                  avgslope_mq3, avgslope_mq136, avgslope_mq137);
    // // --- Read CO2 Sensor---
    // int mhz19_value = -1;
    // if (myMhz19.isReady()) {
    ...
    // --- Create Data Packet ---
    String dataPacket = String(avg_mq3) + "," +
                        String(avg_mq136) + "," +
                        String(avg_mq137) + "," +
                        String(avgslope_mq3) + "," +
                        String(avgslope_mq136) + "," +
                        String(avgslope_mq137) + "," +
                        // String(mhz19_value) + "," +
                        String(counter);
    // --- Send LoRa Packet ---
    LoRa.beginPacket();
