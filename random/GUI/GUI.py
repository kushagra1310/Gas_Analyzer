...
sensor_historical_data = {name: [random.uniform(50, 300) for _ in range(50)] for name in SENSORS}
    ...
    def update_sensor_data(self):
        for name, details in SENSORS.items():
            ...
            if details["status"] == "On":
                new_reading = random.uniform(50, 300)
                sensor_historical_data[name].pop(0)
                sensor_historical_data[name].append(new_reading)
                # Threshold color logic
                threshold = THRESHOLDS.get(name)
                if threshold and new_reading > threshold:
                    data_color = '#FF9900'
                else:
                    data_color = '#ffffff'
                # Sparkline image (pass threshold for color)
                spark_img = create_sparkline(
                    sensor_historical_data[name], width=80, height=28
                )
                card.update_text(
                ...
                card.update_text(
                    status_text="● OFF", status_color="#ff3b3b",
                    data_text="---", data_color="#bdc3c7",
                    sparkline_img=spark_img
                )

        self.status_var.set(
            f"System Status: ONLINE | Last Update: {datetime.now().strftime('%H:%M:%S')}"
        )
